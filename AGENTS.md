...
CCE is local contributor tooling, not a build or CI dependency. The CCE-specific
instructions below apply when its tools are available. If unavailable, use `rg`
and direct file reads, and state that cross-session memory could not be checked.
Keep generated hooks with machine-specific paths in ignored
`.claude/settings.local.json`; do not commit those paths in shared settings.
