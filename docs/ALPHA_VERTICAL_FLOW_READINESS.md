...
| Build the plan | Exact-one-`main.mjs` proposal plus TypeScript `decodeJobProposal`, `resolveJobProposal`, and `constructExecutionPlan` strict passive mechanics | `PASSED` for passive/unwired mechanics | Connect only the selected method-specific adapter and bind admitted limits plus exact runtime/profile identity. |
| Register source and plan | Go `Facade.RegisterPlanV0`/`Facade.GetRegisteredPlanV0` atomic passive fixed-store custody/readback | `PASSED` for the unwired oracle | Expose it only through method-specific authenticated product registration/fetch backed by protected production state. |
