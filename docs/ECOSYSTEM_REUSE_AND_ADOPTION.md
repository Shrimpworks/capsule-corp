...
| Phase 2 fixed snapshot, F1/F2 archive oracle | DB-1 versus current `FixedFileStore` | A production DB would own all Supervisor state, locks, journals, backups, migrations, and corruption responses | **SPIKE-FIRST.** F2 now completes the bounded fixed v2 migration/full-verifier oracle without a new dependency. Do not extend it into an unbounded production engine, and do not select SQLite/driver by reputation alone. | F3-F5 must first complete the fixed archive oracle. Then one storage-engine spike replays the logical/fault/mutation corpus; exact SQLite/driver pin, compile flags, graph, single-writer mode, `FULL` sync, WAL/rollback decision, directory/APFS semantics, integrity/foreign-key checks, online/offline backup, restore/rollback, migration, disk-full, torn/corrupt pages, lock loss, process/power interruption |
...
| Archive/compaction; F1/F2 local oracle complete | Filesystem/APFS primitives now; DB-1 only after logical oracle | Product persistence TCB; retained replay/tombstone authority | **BUILD-NARROWLY** for archive semantics and immutable segment format; **SPIKE-FIRST** for engine mechanics. Generic log compaction may delete referenced history or blur hot/archive index domains. | F2's migration/full verifier is `PASSED`; F3 next owns one-segment activation. Later fixed-oracle and production-store slices must prove complete-cohort atomic move, immutable digest, global indexes, no referenced deletion, restore and mutation behavior |
