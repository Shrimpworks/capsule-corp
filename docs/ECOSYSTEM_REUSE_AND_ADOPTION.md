...
| `.mjs` import/module validation; V0 frames plus unwired V1 artifact | JS-1 plus Proposed ADR-0035's exact Oxc candidate | Planning/approval-understanding TCB when activated; V1 is an unwired parser artifact with zero product consumer/authority | **SPIKE-FIRST** remains: exact Oxc 0.140.0, Rust 1.95.0, and `sha2` 0.10.9 are locked under artifact lock SHA-256 `a79e9fd9fb6c1273c56fdada6eeea02f547b45a4dd8a146264f697fea6bdee45`; **BUILD-NARROWLY** retains the fixed Capsule frame. V1 inventories all 74 locked dependencies and 73 target dependencies, package checksums, source bytes, declared licenses/notices, and SBOM. | V0 is observed; V1 retains exact artifact bytes and same-host reproduction but fails enrollment on independent provenance/signatures and missing vulnerability owner/SLA. V2-V6 still require sandbox/fault evidence, independent consumers, expanded grammar corpus, and runtime no-loader enforcement. Do not handwrite a lexer or treat parser `allow` as launch authority. |
