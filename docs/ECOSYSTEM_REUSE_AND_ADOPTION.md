...
- `FixedFileStore` rewrites a versioned, bounded JSON snapshot with file and directory durability
  barriers. It explicitly is not a production database. Archive F1-F3 and owner-lock G1 are
...
| Phase 2 fixed snapshot, F1-F3 archive oracle | DB-1 versus current `FixedFileStore` | A production DB would own all Supervisor state, locks, journals, backups, migrations, and corruption responses | **SPIKE-FIRST.** F2 completes the bounded fixed v2 migration/full-verifier oracle and F3 completes its first immutable-segment activation without a new dependency. Do not extend it into an unbounded production engine, and do not select SQLite/driver by reputation alone. | F4-F5 must first complete retained lookup/mutation/growth and backup/orphan/reporting in the fixed archive oracle. Then one storage-engine spike replays the logical/fault/mutation corpus; exact SQLite/driver pin, compile flags, graph, single-writer mode, `FULL` sync, WAL/rollback decision, directory/APFS semantics, integrity/foreign-key checks, online/offline backup, restore/rollback, migration, disk-full, torn/corrupt pages, lock loss, process/power interruption |
...
| Archive/compaction; F1-F3 local oracle complete | Filesystem/APFS primitives now; DB-1 only after logical oracle | Product persistence TCB; retained replay/tombstone authority | **BUILD-NARROWLY** for archive semantics and immutable segment format; **SPIKE-FIRST** for engine mechanics. Generic log compaction may delete referenced history or blur hot/archive index domains. | F2's migration/full verifier and F3's first-segment activation are `PASSED`; F4 next owns retained lookup/mutation/growth. Later fixed-oracle and production-store slices must prove complete-cohort atomic move, immutable digest, global indexes, no referenced deletion, restore and mutation behavior |
