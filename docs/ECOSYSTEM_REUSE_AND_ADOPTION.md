...
Status: canonical planning input; research current through 2026-08-05. This document selects
...
- Current authority-path CBOR parsing/encoding remains handwritten and bounded. The retained Gate
  A2 experiment used `fxamacker/cbor` 2.9.1, `cbor2` 2.3.0 plus one transitive package, and
  `thecoolwinter/CBOR` 1.1.2 only as spike inputs. The later production-profile comparison pins
  `fxamacker/cbor` 2.9.2 and `go-cose` 1.3.0 in a standalone experiment. The bounded
  [v0 object-set and wrapper result](V0_CBOR_OBJECT_SET_AND_WRAPPER.md) now adds exact fxamacker
  v2.9.2 plus float16 v0.8.4 to the root module for one unwired `SourceManifest` codec; no existing
  authority package imports it. `go-cose` remains absent and test-only.
- `FixedFileStore` rewrites a versioned, bounded JSON snapshot with file and directory durability
  barriers. It explicitly is not a production database. Archive F1-F4A and owner-lock G1 are
...
| DB-1 | [SQLite 3.53.4](https://sqlite.org/releaselog/3_53_4.html), exact official amalgamation and [F6 candidate comparison](SUPERVISOR_ARCHIVE_F6_SQLITE_EXPERIMENT_PACKET.md), [atomic commit](https://sqlite.org/atomiccommit.html), [backup API](https://sqlite.org/backup.html), and [`integrity_check`](https://sqlite.org/pragma.html#pragma_integrity_check) | Mature, actively maintained, public domain core; exact 2026-07-24 source ID, official archive/`sqlite3.c` SHA3-256, and independently computed SHA-256 retained; no signature/attestation verified | C library on macOS; the research recommendation is an experiment-only Capsule-owned narrow cgo shim with no Go module dependency; maintained cgo and pure-Go alternatives remain measured comparators | `SPIKE-FIRST`; candidate product persistence TCB only after a separate evidence-backed decision. It gains all Supervisor durable authority and therefore requires the strictest native VFS/APFS, restoration, corruption, and power-loss evidence. No dependency is selected or admitted. |
...
| Phase 2 deterministic CBOR predecoder/encoder; handwritten bounded Go code and cross-language fixtures | STD-1 + GO-1 | Product parsing TCB; attacker-controlled proposal/signed-object bytes, no other authority | **ADOPT-PINNED narrowly after each applicable object freezes:** v2.9.2 may replace deterministic typed encoding and field decode only. Keep raw-byte cap/predecode, exact object limits, canonical byte comparison, closed schema/bindings, exact-byte identity, and the handwritten oracle. | [Production comparison](https://github.com/Shrimpworks/capsule-experiments/blob/0d8233b55f153b27a901a9ec45a3834208e3aa86/experiments/production-cbor-cose-profile/RESULTS.md) plus [v0 object-set/wrapper result](V0_CBOR_OBJECT_SET_AND_WRAPPER.md). The unwired root dependency is admitted only for frozen `SourceManifest` v0; plan, registration, approval, Swift, and same-byte consumers remain later gates. |
| COSE_Sign1 wrappers; experiment currently handwrites envelope and Sig_structure | STD-1 + GO-2 + GO-3 | Product crypto-envelope TCB; header parsing/signature input, not key selection or authorization | **TEST-ONLY for go-cose v1.3.0; production NO-GO.** Keep a Capsule-owned Sign1-only wrapper with standard crypto. The candidate saves too little after required caps/header/binding/replay controls and adds broad generic COSE surface plus unresolved support/provenance concerns. | [Production comparison](https://github.com/Shrimpworks/capsule-experiments/blob/0d8233b55f153b27a901a9ec45a3834208e3aa86/experiments/production-cbor-cose-profile/RESULTS.md): 82 approval cases, 2.89M fuzz inputs, exact Sig_structure/equivalent-S, restored header families, trusted-key and binding refusals. Next exact test: production-shaped Swift and same-byte Supervisor/Broker integration |
...
| Phase 2 fixed snapshot, F1-F5 archive oracle | DB-1 versus current `FixedFileStore` | A production DB would own all Supervisor state, locks, journals, backups, migrations, and corruption responses | **SPIKE-FIRST.** F2-F5 complete the bounded fixed-store semantic oracle. The [F6 checkpoint](SUPERVISOR_ARCHIVE_F6_SQLITE_EXPERIMENT_PACKET.md) recommends official SQLite 3.53.4 plus an experiment-only narrow Capsule cgo shim for comparison; it does not select or admit either. | Separately authorize one `capsule-experiments` execution that preserves split active/immutable-segment databases and replays the exact F2-F5 logical/fault/mutation/archive/backup/restore-admission/orphan corpus in fixed rollback-journal and WAL configurations, plus native VFS/result-code and real macOS/APFS/power-loss gates. Then record a separate decision ADR. |
...
| Archive/compaction; F1-F5 local oracle complete | Filesystem/APFS primitives now; DB-1 only after logical oracle | Product persistence TCB; retained replay/tombstone authority | **BUILD-NARROWLY** for archive semantics and immutable segment format; **SPIKE-FIRST** for engine mechanics. Generic log compaction or one monolithic database may delete referenced history or erase the F3 publication/orphan boundary. | F2-F5 are `PASSED` in their exact fixed-oracle scopes. F6 execution and engine selection remain `BLOCKED`; the [execution checkpoint](SUPERVISOR_ARCHIVE_F6_SQLITE_EXPERIMENT_PACKET.md) requires split mutable active and content-addressed immutable segment databases, complete-cohort atomic move, global indexes, no referenced deletion, and exact restore/mutation behavior. |
...
| `.mjs` import/module validation; V0 frames, unwired V1 artifact, blocked V2 profile, accepted R0 replacement architecture, and passive R1 role contracts | JS-1 plus Accepted ADR-0035's exact Oxc candidate and ADR-0036; APL-4 for App Sandbox/Hardened Runtime; APL-2 for the two role-specific private XPC launchers | Planning/approval-understanding TCB when activated; V1 is an unwired parser artifact, V2 is a test-only local process/fault checkpoint, and R0/R1 are accepted design plus passive bytes with zero product consumer/authority | **SPIKE-FIRST** remains: exact Oxc 0.140.0, Rust 1.95.0, and `sha2` 0.10.9 are locked under artifact lock SHA-256 `a79e9fd9fb6c1273c56fdada6eeea02f547b45a4dd8a146264f697fea6bdee45`; **BUILD-NARROWLY** retains role-specific fixed Capsule frames; **ADOPT-PLATFORM** permits only supported App Sandbox, Hardened Runtime, launch/library constraints, private XPC, `posix_spawn`, rlimits, and `proc_pid_rusage`. Direct parent inheritance, shared/generic services/results/caches, private jetsam/spawn APIs, deprecated custom profiles, app-group/key sharing, and generic XPC/JSON buses are rejected. V1 inventories all 74 locked dependencies and 73 target dependencies, package checksums, source bytes, declared licenses/notices, and SBOM. V2/R0 add no package dependency. | R1/R2 and signed inactive R3 are `PASSED` in their exact scopes. Exact R4-v1 candidates are `NO_GO`; R4-v2 is unexecuted and `BLOCKED`. ADR-0040 makes the validator post-alpha defense-in-depth and M2/S1 no longer waits on R5. Resume only with a supportable child-lifetime/residue contract. Parser `allow` never grants launch authority. |
...
| C5b19 prospective Supervisor control/storage lane exchange | Darwin anonymous `pipe`, `fcntl`, `poll`, continuous-time and child-status APIs plus Capsule fixed framing | Test-only candidate; storage-message transport has no process authority, which remains solely in the Supervisor control lane | **ADOPT-PLATFORM / BUILD-NARROWLY** for the [gate-1 candidate](C5B19_GATE1_MECHANISM_DECISION.md), pending independent review. No generic bus, third-party queue, cross-language atomic-slot library or product admission. | C5b19 gates 1–5; exact at-most-one frame/operation, nonblocking control, independent stop under blocked store, `SIGCHLD`/exclusive-waiter closure, continuous-clock conversion, error/late-reply/exit-race mutations and authorized benign run evidence |
...
ADOPT NOW (standards/platform/test choices; no product activation)
  APL-1..6 + GO-3 + STD-1 + TEST-1/RUST-1 + SUP-1 formats
       ...
       +--> F2 fixed-store v2 migration/full-verifier oracle (`PASSED` local scope)
...
BOUNDED DECISIONS
  ...
  [SQLite production-engine comparison] <-- completed F2/G2 prerequisites + F3-F5 logical oracle
