...
| Phase 2 fixed snapshot, F1-F4A archive oracle | DB-1 versus current `FixedFileStore` | A production DB would own all Supervisor state, locks, journals, backups, migrations, and corruption responses | **SPIKE-FIRST.** F2 completes the bounded fixed v2 migration/full-verifier oracle, F3 completes its first immutable-segment activation, and F4A completes read-only retained routing without a new dependency. Do not extend it into an unbounded production engine, and do not select SQLite/driver by reputation alone. | F4B-F5 must first complete mutation/new tombstones, bounded growth, and backup/orphan/reporting in the fixed archive oracle. Then one storage-engine spike replays the logical/fault/mutation corpus; exact SQLite/driver pin, compile flags, graph, single-writer mode, `FULL` sync, WAL/rollback decision, directory/APFS semantics, integrity/foreign-key checks, online/offline backup, restore/rollback, migration, disk-full, torn/corrupt pages, lock loss, process/power interruption |
...
| Archive/compaction; F1-F4A local oracle complete | Filesystem/APFS primitives now; DB-1 only after logical oracle | Product persistence TCB; retained replay/tombstone authority | **BUILD-NARROWLY** for archive semantics and immutable segment format; **SPIKE-FIRST** for engine mechanics. Generic log compaction may delete referenced history or blur hot/archive index domains. | F2 migration, F3 first activation, and F4A read-only retained routing are `PASSED`; F4B next owns atomic mutation/new tombstones and F4C owns bounded growth. Later fixed-oracle and production-store slices must prove complete-cohort atomic move, immutable digest, global indexes, no referenced deletion, restore and mutation behavior |
...
| `.mjs` import/module validation; V0 frames, unwired V1 artifact, blocked V2 profile, and passed replacement design | JS-1 plus Proposed ADR-0035's exact Oxc candidate; APL-4 for App Sandbox/Hardened Runtime; APL-2 only if the launcher topology is accepted | Planning/approval-understanding TCB when activated; V1 is an unwired parser artifact, V2 is a test-only local process/fault checkpoint, and the replacement is design/public-SDK evidence with zero product consumer/authority | **SPIKE-FIRST** remains: exact Oxc 0.140.0, Rust 1.95.0, and `sha2` 0.10.9 are locked under artifact lock SHA-256 `a79e9fd9fb6c1273c56fdada6eeea02f547b45a4dd8a146264f697fea6bdee45`; **BUILD-NARROWLY** retains the fixed Capsule frame; **ADOPT-PLATFORM** permits only supported App Sandbox, Hardened Runtime, launch/library constraints, XPC, `posix_spawn`, rlimits, and `proc_pid_rusage`. Direct parent-sandbox inheritance, private jetsam/spawn APIs, deprecated custom profiles, app-group/key sharing, and generic XPC/JSON buses are rejected. V1 inventories all 74 locked dependencies and 73 target dependencies, package checksums, source bytes, declared licenses/notices, and SBOM. V2/replacement add no package dependency. | V0 is observed; V1 retains exact bytes but fails enrollment. V2 is `BLOCKED`. The passed replacement review rejects direct inherited helpers and identifies only a separately App-Sandboxed method-specific XPC launcher owning one fresh parser child. R0 remains blocked on topology, the writable private container versus no-store policy, and hard memory: `RLIMIT_AS` fails, the public footprint setter returned `KERN_NO_ACCESS`, and `proc_pid_rusage` kill is reactive. Resume through R0-R5 with newly versioned protocol/process/artifact identities and separately authorized signed evidence. V3-V6 still require independent consumers, expanded grammar/fault evidence, and runtime no-loader enforcement. Parser `allow` never grants launch authority. |
