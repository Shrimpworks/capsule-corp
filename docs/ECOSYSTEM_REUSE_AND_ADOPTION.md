...
2. **Adopt standards and small implementations, not generic envelopes.** RFC 8949 deterministic
   ...
   inputs or oracles. Production CBOR/COSE should use a pinned, bounded library behind
   Capsule-owned object profiles; the current handwritten encoders and scanners remain useful
   conformance oracles until that comparison closes.
...
- Current production-shaped CBOR parsing/encoding is deliberately handwritten and bounded. The
  retained Gate A2 experiment used `fxamacker/cbor` 2.9.1, `cbor2` 2.3.0 plus one transitive
  package, and `thecoolwinter/CBOR` 1.1.2 only as spike inputs.
...
| GO-1 | [`fxamacker/cbor` v2.9.2](https://github.com/fxamacker/cbor/releases/tag/v2.9.2) | Active; 2026-05-04 release; MIT; signed GitHub release commit; reports fuzzing and prior external assessments | Go 1.20+; one direct module and its graph must be recorded by the spike | Candidate product parsing TCB; receives bounded signed-object bytes, no network/filesystem/process authority |
| GO-2 | [`veraison/go-cose` v1.3.0](https://github.com/veraison/go-cose/releases/tag/v1.3.0) | Active community project; 2024-09-27 release; MPL-2.0; SECURITY policy; provenance/signing of release asset is unknown | Go; direct/transitive graph must be measured | Candidate product cryptographic-envelope TCB; parses headers and constructs signature inputs but must not select keys or authorization |
...
| Phase 2 deterministic CBOR predecoder/encoder; handwritten bounded Go code and cross-language fixtures | STD-1 + GO-1 | Product parsing TCB; attacker-controlled proposal/signed-object bytes, no other authority | **SPIKE-FIRST**, likely **ADOPT-PINNED** behind object wrappers. A maintained decoder removes broad format code, but generic decode alone does not enforce Capsule canonical-on-wire, limits, unknown-field, or binding rules. | Phase 2 production profile spike; compare v2.9.2 with current scanner across all fixtures, sustained fuzzing, CPU/RSS caps, canonical re-encode, duplicate/unknown/tag/trailing cases, exact module graph/license/provenance; retain handwritten oracle until mutation parity |
| COSE_Sign1 wrappers; experiment currently handwrites envelope and Sig_structure | STD-1 + GO-2 + GO-3 | Product crypto-envelope TCB; header parsing/signature input, not key selection or authorization | **SPIKE-FIRST.** Compare pinned `go-cose` with a reviewed narrow wrapper. Adopt only if disabled features and graph are smaller than maintaining the exact wrapper. | ADR-0019 production profile; exact protected-header allowlist, empty unprotected fields, raw signature policy, external-AAD/detached/embedded-key/URL refusal, wrong-purpose/audience/binding corpus and independent review |
...
| `.mjs` import/module validation; no implementation | JS-1 plus parser selected by active task | Planning TCB; parses hostile source but must gain no execution/network/filesystem/process authority | **SPIKE-FIRST.** Parser selection is owned by active task **Select .mjs parser boundary**. Do not handwrite lexer/regex scanner or reuse full runtime in-process merely for parsing. | Active parser task; bounded/out-of-process or otherwise confined parser, exact AST refusals, resource corpus, Test262-derived lexical cases. M1 consumes the decision; runtime no-loader remains the security control |
...
SPIKE NEXT (three bounded decisions)
  [CBOR/COSE production profile]
