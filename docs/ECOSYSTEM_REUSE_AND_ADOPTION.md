...
| `.mjs` import/module validation; V0 frames, unwired V1 artifact, blocked V2 profile, and accepted R0 replacement architecture | JS-1 plus Accepted ADR-0035's exact Oxc candidate and ADR-0036; APL-4 for App Sandbox/Hardened Runtime; APL-2 for the two role-specific private XPC launchers | Planning/approval-understanding TCB when activated; V1 is an unwired parser artifact, V2 is a test-only local process/fault checkpoint, and R0 is accepted design/public-SDK evidence with zero product consumer/authority | **SPIKE-FIRST** remains: exact Oxc 0.140.0, Rust 1.95.0, and `sha2` 0.10.9 are locked under artifact lock SHA-256 `a79e9fd9fb6c1273c56fdada6eeea02f547b45a4dd8a146264f697fea6bdee45`; **BUILD-NARROWLY** retains role-specific fixed Capsule frames; **ADOPT-PLATFORM** permits only supported App Sandbox, Hardened Runtime, launch/library constraints, private XPC, `posix_spawn`, rlimits, and `proc_pid_rusage`. Direct parent inheritance, shared/generic services/results/caches, private jetsam/spawn APIs, deprecated custom profiles, app-group/key sharing, and generic XPC/JSON buses are rejected. V1 inventories all 74 locked dependencies and 73 target dependencies, package checksums, source bytes, declared licenses/notices, and SBOM. V2/R0 add no package dependency. | V0/V1/V2 evidence remains unchanged and V2 is `BLOCKED`. ADR-0036 completes R0 with two role-specific private launchers, residual private-container scratch plus mandatory cleanup, and an evidence-derived reactive watermark that is not a hard peak/exact cap or host-availability guarantee. Resume sequentially through R1 passive contracts/field authority, R2 unsigned construction, separately authorized R3 signing/install, R4 confinement/resource/residue corpus, R5D daemon consumer, R5B Broker consumer, then M2/S1. Threshold/cadence/overshoot values remain unset until R4. Parser `allow` never grants launch authority. |
