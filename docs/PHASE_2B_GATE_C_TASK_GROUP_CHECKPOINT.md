...
| --- | --- | --- |
| `PASSED` local conformance slices / `IN_PROGRESS — TRENDING_GOOD` parent workstream | Archive F2/F3 / F4+ | F2's explicit v1-to-v2 migration/full verifier and F3's first immutable-segment prepare/verify/activate transaction pass with exact missing-history preservation, visible tombstones, publish-before-reference ordering, complete old-or-new reopen, and zero adapter calls. Continue next with F4 retained lookup/mutation/growth; production archive/store admission remains later. |
