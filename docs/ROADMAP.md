...
3. preserve the passed v20 no-launch denial-test materialization and its fail-closed pre-ready
   runtime refusal, then use the passed v21 diagnostic materialization to identify the exact stage
...
work. The fixed guest is not product alpha. V20 refused before readiness without launching a guest;
the v21 diagnostic successor is materialized but unexecuted. The hostile owner-only internal alpha
is the next product checkpoint, and external alpha remains a later distinct gate.
