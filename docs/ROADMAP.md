...
the first unwired slice now implements strict TypeScript raw/schema proposal decoding and strict Go
exact-byte plan/registration wrappers. Semantic planning, Supervisor registration state, and Swift
remain pending. The next work is the semantic planner, exact registration, and then the registered-
plan/fake-backend lifecycle. Public cutover waits for that vertical slice, an atomic
