...
| Product Source Validator V1-V5 and downstream M2/S1 consumers | `BLOCKED` | V0 and the unwired V1 artifact passed their bounded scopes, but the exact V2 macOS profile cannot enforce `RLIMIT_AS`, the unsandboxed mechanism retains file/socket/write authority, and the supported App Sandbox child entitlement shape changes the fixed V1 bytes. Resume only with a newly reviewed/enrolled artifact and supported memory/confinement design; independent daemon/Broker invocation and runtime no-loader enforcement remain later gates. |
