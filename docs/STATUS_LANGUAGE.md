...
| Authenticated-local-IPC S3 native contract | `PASSED` | Exact XPC dictionary keys/types, numeric method/status/reason tags, versions, key counts, caps, configured deadlines, refusal precedence, replay, and response-loss fixtures remain frozen for the original three passive methods. C2a before/at/after equality fixtures for all three deadlines, the native harness, listener, peer authentication, installed profile, consumers, and product IPC remain `BLOCKED`. |
