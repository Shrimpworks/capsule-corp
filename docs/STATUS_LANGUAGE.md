...
| Fixed denial-test v21 diagnostic materialization | `PASSED` | A/B runner/profile/controller reproduction, strict signatures, independent composed-digest calculation, C17 warning-as-error builds, Go tests/vet, and false/blocked authority assertions passed for a fixed pre-ready-stage/early-stderr diagnostic successor. No v21 runner, libkrun/HVF process, or guest was executed. Dynamic v21 execution is `BLOCKED` on fresh exact one-use authorization. |
...
| Governed `deno_core` runtime engineering | `IN_PROGRESS — TRENDING_BAD` | Physical omission, same-host reproduction, standalone root, fork-native Linux/arm64 reconstruction, governed-fork promotions, C1, passive C2A, C2B v1-v4, v19 fixed-owned-guest execution, v20 no-launch materialization, and v21 diagnostic materialization passed in their scopes. The exact v20 runner then refused before readiness without a guest launch, and missing early stderr left the stage unknown. V21 dynamic diagnosis, typed transport, hostile-source composition, installed evidence, and admission remain. |
...
| Governed libkrun source and local library hardening | `IN_PROGRESS — TRENDING_BAD` | The governed r3 backport, immutable default transition, bounded library/custody/transport slices, and one exact fixed benign guest composition passed. V20 then refused before readiness without enough persisted diagnostic evidence to identify the check or libkrun stage. V21 diagnosis, final typed transport/launcher profile, denial and hostile corpora, independent product-admission review, installed composition, and final profile reruns remain. |
