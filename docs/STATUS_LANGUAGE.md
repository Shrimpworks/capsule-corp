...
| Fixed denial-test v27 passive-network correction | `PASSED` | Network-disabled A/B roots and independently reproduced runner/signature/profile/digest/controller evidence passed for accepting only loopback plus optional down/unbacked `dummy0`, rejecting virtio backing and non-loopback/default routes, and retaining no-connect/no-send semantics. No v27 runner, libkrun/HVF process, or guest was executed. Dynamic v27 execution is `BLOCKED` on fresh exact one-use authorization. |
...
| Governed `deno_core` runtime engineering | `IN_PROGRESS — TRENDING_GOOD` | Physical omission, same-host reproduction, standalone root, fork-native Linux/arm64 reconstruction, governed-fork promotions, C1, passive C2A, C2B v1-v4, v19 fixed-owned-guest execution, v20-v27 materializations, v23 root-digest localization, v24 early denials, and v26 consolidated localization passed in their scopes. V25 is `NO_GO`; v27 execution, later controls, typed transport, hostile-source composition, installed evidence, and admission remain. |
...
| Governed libkrun source and local library hardening | `IN_PROGRESS — TRENDING_GOOD` | The governed r3 backport, immutable default transition, bounded library/custody/transport slices, exact fixed benign composition, v24 early denials, and v26 active local-CID/raw-block denials passed. V26 localized the next stop to expected passive inventory without connecting or sending; v27 corrects that policy. V27 execution, final typed transport/launcher profile, later denial and hostile corpora, independent product-admission review, installed composition, and final profile reruns remain. |
