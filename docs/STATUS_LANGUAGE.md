...
| Fixed denial-test v23 hash-diagnostic materialization | `PASSED` | Four matching signed runner builds, A/B root/profile equality, strict signature/entitlement checks, independent composed-digest calculation, Go tests/vet, clean controller builds, and authority assertions passed for staged-path/open-FD and bounded runner-stage/mismatch hashing. No v23 runner, libkrun/HVF process, or guest was executed. Dynamic v23 execution is `BLOCKED` on fresh exact one-use authorization. |
...
| Governed `deno_core` runtime engineering | `IN_PROGRESS — TRENDING_BAD` | Physical omission, same-host reproduction, standalone root, fork-native Linux/arm64 reconstruction, governed-fork promotions, C1, passive C2A, C2B v1-v4, v19 fixed-owned-guest execution, and v20-v23 no-launch materializations passed in their scopes. V22 localized the repeated pre-ready refusal to inherited-root SHA-256 while host-only hashing matches. V23 diagnosis, typed transport, hostile-source composition, installed evidence, and admission remain. |
...
| Governed libkrun source and local library hardening | `IN_PROGRESS — TRENDING_BAD` | The governed r3 backport, immutable default transition, bounded library/custody/transport slices, and one exact fixed benign guest composition passed. V22 localized the later no-launch stop to root SHA-256 before libkrun configuration, but host-only hashing did not reproduce the mismatch. V23 diagnosis, final typed transport/launcher profile, denial and hostile corpora, independent product-admission review, installed composition, and final profile reruns remain. |
