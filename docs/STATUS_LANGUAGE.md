...
| Fixed denial-test v24 corrected materialization | `PASSED` | Four matching signed runner builds, A/B profile equality, strict signature/entitlement checks, independent composed-digest calculation, Go tests/vet, clean controller builds, literal parsing, and authority assertions passed after replacing the byte array with the exact lowercase root digest plus a C17 static length assertion. No v24 runner, libkrun/HVF process, or guest was executed. Dynamic v24 execution is `BLOCKED` on fresh exact one-use authorization. |
...
| Governed `deno_core` runtime engineering | `IN_PROGRESS — TRENDING_GOOD` | Physical omission, same-host reproduction, standalone root, fork-native Linux/arm64 reconstruction, governed-fork promotions, C1, passive C2A, C2B v1-v4, v19 fixed-owned-guest execution, v20-v24 no-launch materializations, and v23 root-digest localization passed in their scopes. V23 confirmed a malformed expected-digest fixture; v24 corrects it reproducibly. V24 dynamic execution, typed transport, hostile-source composition, installed evidence, and admission remain. |
...
| Governed libkrun source and local library hardening | `IN_PROGRESS — TRENDING_GOOD` | The governed r3 backport, immutable default transition, bounded library/custody/transport slices, and one exact fixed benign guest composition passed. V23 proved the later no-launch stop was a malformed expected-digest fixture before libkrun configuration, not a root mutation. V24 dynamic execution, final typed transport/launcher profile, denial and hostile corpora, independent product-admission review, installed composition, and final profile reruns remain. |
