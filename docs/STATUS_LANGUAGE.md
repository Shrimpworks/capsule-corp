...
| Fixed denial-test v25 vsock-diagnostic materialization | `PASSED` | Network-disabled A/B root builds, four matching signed runners, A/B profiles, independent composed-digest calculation, Go tests/vet, clean controller builds, and authority assertions passed for distinct no-connect/no-send socket/refusal/device markers. No v25 runner, libkrun/HVF process, or guest was executed. Dynamic v25 execution is `BLOCKED` on fresh exact one-use authorization. |
...
| Governed `deno_core` runtime engineering | `IN_PROGRESS — TRENDING_GOOD` | Physical omission, same-host reproduction, standalone root, fork-native Linux/arm64 reconstruction, governed-fork promotions, C1, passive C2A, C2B v1-v4, v19 fixed-owned-guest execution, v20-v25 materializations, v23 root-digest localization, and v24 early denial controls passed in their scopes. V25 diagnosis, later hostile controls, typed transport, hostile-source composition, installed evidence, and admission remain. |
...
| Governed libkrun source and local library hardening | `IN_PROGRESS — TRENDING_GOOD` | The governed r3 backport, immutable default transition, bounded library/custody/transport slices, exact fixed benign composition, and v24 corrected/early-denial guest execution passed. V24 localized the next stop to the vsock-check family without connecting or sending. V25 diagnosis, final typed transport/launcher profile, later denial and hostile corpora, independent product-admission review, installed composition, and final profile reruns remain. |
