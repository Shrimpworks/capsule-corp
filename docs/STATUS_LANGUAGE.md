...
| macOS installation and distribution | `IN_PROGRESS — TRENDING_GOOD` | I0's exact inactive one-app/seven-role profile and I1A's unsigned construction are `PASSED`. I2A also `PASSED` the one-shot Coordinator-authorized/Supervisor-created protected-root decision and passive contract. I1B and installed I2B-I4 setup/protected-root/IPC/manual-replacement evidence remain blocked; automatic updates, Developer ID distribution, a support-floor matrix, and complete uninstall are later gates. |
