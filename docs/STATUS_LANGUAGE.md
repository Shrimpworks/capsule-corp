...
| Installed owner-lock G3/I2B | `BLOCKED` | Proposed ADR-0038 selects the bootstrap owner and signed-record/store-open contract. I1B and I2B1 passive request/record objects are `PASSED`; resume with I2B2 unsigned construction, then separately authorized exact Team-3DDR profiles and the installed handoff/container/descriptor-relative fault corpus. |
| Archive fixed-store F2-F4B | `PASSED` | F2 migration/full verification, F3 first-segment activation, F4A retained lookup, and F4B atomic authority/lifecycle mutation pass their exact local fixed-store scopes. F4B adds the independent append-only effect-tombstone source, same-transaction intent issuance, durable exact replay, retained-global collision reconstruction, and historical `superseded-by-current` lookup without inventing an issuing lifecycle. See the [F4B result](SUPERVISOR_ARCHIVE_F4B_MUTATION_RESULT.md). |
| Archive fixed-store F4C-F6 and product admission | `IN_PROGRESS — TRENDING_GOOD` | F4C bounded multi-segment growth, F5 backup/restore and cleanup policy, F6 production-engine selection, real power-loss evidence, and every product consumer remain open. F4B does not advance those claims. |
...
| macOS installation and distribution | `IN_PROGRESS — TRENDING_GOOD` | I0's exact inactive one-app/seven-role profile, I1A's unsigned construction, I1B's exact development-signed execution-disabled composition, I2A's one-shot Coordinator-authorized/Supervisor-created protected-root decision, and the post-I1B Apple-platform semantics research are `PASSED` in their exact scopes. Installed I2B-I4 protected-root/IPC/manual-replacement evidence remains `BLOCKED`; automatic updates, Developer ID distribution, a support-floor matrix, and complete uninstall are later gates. |
