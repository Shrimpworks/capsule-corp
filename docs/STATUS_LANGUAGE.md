...
| Fake-backend alpha vertical flow | `BLOCKED` | Passive source/plan bytes, the pure unwired proposal resolver/plan builder, and unwired registration, approval/attempt, lifecycle, and fake-backend mechanics exist, but no selected submission adapter, product Source Validator consumers, daemon builder consumer, authenticated IPC facades, Broker render/sign flow, production approval verifier, or receipt/summary compositor connects them. See the [readiness map](ALPHA_VERTICAL_FLOW_READINESS.md). |
...
| Installed owner-lock G3/I2B | `BLOCKED` | Proposed ADR-0038 selects the bootstrap owner and signed-record/store-open contract. I1B, I2B1 passive request/record objects, and I2B2 unsigned installation-only construction are `PASSED` in their exact scopes; resume only after production wrapper review with separately authorized I2B3 exact Team-3DDR signing/key/App Group/service/container handoff evidence, then the descriptor-relative fault corpus. |
| Archive fixed-store F2-F4C | `PASSED` | F2 migration/full verification, F3 first-segment activation, F4A retained lookup, F4B atomic authority/lifecycle mutation, and F4C bounded second/later immutable-segment growth pass their exact local fixed-store scopes. F4C retains the independent effect source and complete cohorts through deterministic generation/checkpoint ancestry, exact segment 64 acceptance, segment 65 refusal, and old-or-complete-new activation. See the [F4C result](SUPERVISOR_ARCHIVE_F4C_GROWTH_RESULT.md). |
| Archive fixed-store F5-F6 and product admission | `IN_PROGRESS — TRENDING_GOOD` | F5 backup/restore and orphan-cleanup policy, F6 production-engine selection, real power-loss evidence, and every product consumer remain `BLOCKED`. F4C does not advance those claims. |
...
| Governed `deno_core` C2B passive bindings | `PASSED` | Immutable v1 preserves its exact historical draft-head checkpoint. The separate 7,115-byte v2 successor preserves C1/C2A/v1 and binds merged capsule-experiments PR #4, six no-guest constructed identities, the unadmitted manifest candidate, and the non-final preflight harness with 120 recursive field classifications and zero effects. Final runner, separate firmware, composed profile, unsupported resource limits, guest evidence, and admission stay null/refusing; composed-profile/guest execution remains `BLOCKED`. |
...
| Governed `deno_core` runtime engineering | `IN_PROGRESS — TRENDING_GOOD` | Physical omission, same-host reproduction, standalone root, fork-native Linux/arm64 reconstruction, governed-fork acceptance/defaults, C1, passive C2A, and C2B no-guest build closure passed in their exact scopes; a separately authorized C2B owned-guest task owns the first composed-profile execution evidence. |
