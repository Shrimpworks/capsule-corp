...
| Owner-only hostile-`.mjs` internal alpha | `IN_PROGRESS — TRENDING_GOOD` | The scope and ordered gates are frozen by ADR-0040. Atomic source custody, authenticated IPC, Broker render/sign, production approval verification, protected installed state, real adapter/recovery, runtime/profile admission, and completion composition remain `BLOCKED`. See the [readiness map](ALPHA_VERTICAL_FLOW_READINESS.md). |
| Fake-backend authority integration | `BLOCKED` | Passive source/plan bytes and unwired registration, approval/attempt, lifecycle, and FakeBackend mechanics exist, but no selected adapter, narrow product proposal, atomic source custody, authenticated facades, Broker flow, production verifier, or compositor connects them. It no longer waits on Source Validator R4/R5. |
...
| Governed `deno_core` C2B passive bindings and host-runner source contract | `PASSED` | Immutable v1/v2/v3 preserve their exact historical and passive successor scopes. The later dependency-free C17 source contract freezes one Supervisor-owned runner per AttemptID, FDs 0–7, raw-root FD 4, three console ports, explicit implicit-device disable calls, exact ordering, forbidden authority, and external teardown with zero effects. It is not a final runner: accepted header/current-source dylib inputs, exact ABI build bytes/digest, a new materialized profile, guest evidence, and admission remain `BLOCKED`. |
