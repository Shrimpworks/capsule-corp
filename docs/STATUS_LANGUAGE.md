...
| Authenticated-local-IPC S3 native contract | `PASSED` | Exact XPC dictionary keys/types, numeric method/status/reason tags, versions, key counts, caps, deadlines, refusal precedence, replay, and response-loss fixtures remain frozen for the original three passive methods. The native harness, listener, peer authentication, installed profile, consumers, and product IPC remain `BLOCKED`. |
| Authenticated-local-IPC C4 passive evidence claim | `BLOCKED` | PR #247 merged the two-method approval/attempt candidate, but the completed CL4 audit has disposition `AMEND`. A separate focused evidence-hardening PR must add explicit before/at/after 5,000-ms cases and stronger independent complete-dictionary, closed-map, all-field, required-`noState`, cancellation/deadline, and refusal-table verification. No runtime authority bypass was found; installed and product status remain `BLOCKED`. |
| CL4 IPC refusal-matrix audit | `PASSED` | The independent read-only audit completed with disposition `AMEND`, found no runtime authority bypass, and identified the focused evidence-hardening implementation required before the C4 passive claim can pass. CL4 itself is not the follow-up implementation. No ADR change follows. |
