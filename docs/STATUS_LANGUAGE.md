...
| Product Source Validator V1-V5 and downstream M2/S1 consumers | `BLOCKED` | V0 and the unwired V1 artifact passed their bounded scopes. V2 cannot enforce `RLIMIT_AS`; the unsandboxed mechanism retains ambient authority. The passed replacement-design slice rejects direct App Sandbox inheritance because it preserves parent rights and identifies only a separately sandboxed XPC-launcher/fresh-child candidate. That topology is unselected, App Sandbox grants a writable private container, and the public footprint setter returned `KERN_NO_ACCESS`; reactive monitoring is not a hard peak cap. Resume through R0-R5 with new identities and separately authorized signed evidence. Independent daemon/Broker invocation and runtime no-loader enforcement remain later gates. |
...
| Archive fixed-store F2 migration and F3 first-segment activation | `PASSED` | F2's owner-asserted v1-to-v2 migration/full verifier and F3's sealed immutable-segment prepare/verify/publish/activate transaction pass their exact local scopes, including missing-history preservation, publish-before-reference ordering, atomic old-or-new reopen, complete visible tombstones, valid-orphan reporting, and retained fault/corruption/substitution/concurrency/death oracles. F4+ lookup/mutation/backup/engine behavior and production admission remain outside this scope. |
