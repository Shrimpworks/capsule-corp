...
| Fixed denial-test v22 convergence materialization | `PASSED` | A/B runner/profile/controller reproduction, strict signatures, independent composed-digest calculation, Go tests/vet, clean controller builds, and authority assertions passed for a successor that converges every non-`R` result through bounded child wait, stderr persistence, waitpid recording, and canary verification. No v22 runner, libkrun/HVF process, or guest was executed. Dynamic v22 execution is `BLOCKED` on fresh exact one-use authorization. |
...
| Governed `deno_core` runtime engineering | `IN_PROGRESS — TRENDING_BAD` | Physical omission, same-host reproduction, standalone root, fork-native Linux/arm64 reconstruction, governed-fork promotions, C1, passive C2A, C2B v1-v4, v19 fixed-owned-guest execution, and v20-v22 no-launch materializations passed in their scopes. V20 and v21 then refused before readiness without a guest launch; v21 still returned before authoritative stderr/waitpid evidence persisted. V22 dynamic diagnosis, typed transport, hostile-source composition, installed evidence, and admission remain. |
...
| Governed libkrun source and local library hardening | `IN_PROGRESS — TRENDING_BAD` | The governed r3 backport, immutable default transition, bounded library/custody/transport slices, and one exact fixed benign guest composition passed. V20 and v21 then refused before readiness without enough persisted child/stderr/waitpid evidence to identify the check or libkrun stage. V22 diagnosis, final typed transport/launcher profile, denial and hostile corpora, independent product-admission review, installed composition, and final profile reruns remain. |
