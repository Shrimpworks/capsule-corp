...
| Installed owner-lock G3/I2B | `BLOCKED` | Proposed ADR-0038 selects the bootstrap owner and signed-record/store-open contract. I1B, I2B1 passive objects, the narrow production-shaped wrapper review, I2B2 unsigned construction, and the I2B3 exact-profile/signing preflight are `PASSED` in their exact scopes. The required stale-profile stop test then proved that the still-valid I1B Supervisor profile can rewrite current-profile state in the stable Supervisor App Sandbox container. Resume only after an ADR selects and provisions a container/signing-identity epoch boundary, then separately authorize the remaining Keychain/service/root corpus. |
...
| Governed `deno_core` runtime engineering | `IN_PROGRESS — TRENDING_GOOD` | Physical omission, same-host reproduction, standalone root, fork-native Linux/arm64 reconstruction, governed-fork promotions, C1, passive C2A, C2B v1-v3, and v4 build/static materialization passed in their exact scopes. A separately authorized fixed-owned-guest experiment naming v4's exact composed digest is the next guest boundary. |
...
| macOS installation and distribution | `IN_PROGRESS — TRENDING_BAD` | I0-I2B2 and the I2B3 exact Team-3DDR profile/signing preflight are `PASSED` in their exact scopes. The required I2B3 stale-profile test found that profile/CDHash/App-Group/Keychain-group rotation does not version the stable Supervisor private container: an archived I1B Supervisor profile rewrote current-profile state. Installed I2B-I4 remains `BLOCKED` pending an ADR-selected identity/container epoch; automatic updates, Developer ID distribution, a support-floor matrix, and complete uninstall are later gates. |
