...
| Authenticated-local-IPC S3 native contract | `PASSED` | Exact XPC dictionary keys/types, numeric method/status/reason tags, versions, key counts, caps, refusal precedence, replay, and response-loss fixtures remain frozen for the original three passive methods. C2a now retains strict 9,999/10,000/10,001-, 4,999/5,000/5,001-, and 1,999/2,000/2,001-ms cases with equality expired and complete Go/Node agreement. The native harness, listener, peer authentication, installed profile, consumers, and product IPC remain `BLOCKED`. |
...
| Typed guest transport research and C5b0 packet | `PASSED` | The passive R2 design and C5a contract select three role-distinct attempt-bound source/input/completion streams and exact byte/state/restoration rules. The immutable [C5b0 archive](https://github.com/Shrimpworks/capsule-experiments/tree/b357d0c0fb29100c180494e67cebd7809aabe3c5/experiments/typed-guest-transport-c5b0-v19-successor) now binds the v19 lineage, governed 103-byte source/SourceManifest, five role contracts, no-run plan/profile, and fresh frames. Its executable identities remain null; fresh executable construction, controlled execution, installed composition, and admission remain `BLOCKED`. |
...
| Installed owner-lock G3/I2B | `BLOCKED` | Proposed ADR-0038 selects the bootstrap owner and signed-record/store-open contract. Proposed ADR-0045 selects a versioned Supervisor-authority-epoch candidate, and the immutable [C3a E0 archive](https://github.com/Shrimpworks/capsule-experiments/tree/dee784d40684100f8315720fab9a5cd3399f492b/experiments/macos-installation-i2b3-supervisor-authority-epoch-e0) now `PASSED` exact unsigned reproducible construction and mutation verification. The stable Supervisor identity remains legacy residue. Resume with separately authorized Apple Development C3b/E1 identity-separation mutations, then stop again before the later Keychain/service/root corpus. ADR-0045 remains Proposed. |
