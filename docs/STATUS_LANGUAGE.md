...
| Fixed denial-test v26 consolidated materialization | `PASSED` | Network-disabled A/B roots and independently reproduced runner/signature/profile/digest/controller evidence passed for a no-connect/no-send `VM_SOCKETS_GET_LOCAL_CID` control plus fixed failure-detail and bounded console-progress retention. No v26 runner, libkrun/HVF process, or guest was executed. Dynamic v26 execution is `BLOCKED` on fresh exact one-use authorization. |
...
| Governed `deno_core` runtime engineering | `IN_PROGRESS — TRENDING_GOOD` | Physical omission, same-host reproduction, standalone root, fork-native Linux/arm64 reconstruction, governed-fork promotions, C1, passive C2A, C2B v1-v4, v19 fixed-owned-guest execution, v20-v26 materializations, v23 root-digest localization, and v24 early denial controls passed in their scopes. V25 is `NO_GO`; v26 execution, later hostile controls, typed transport, hostile-source composition, installed evidence, and admission remain. |
...
| Governed libkrun source and local library hardening | `IN_PROGRESS — TRENDING_GOOD` | The governed r3 backport, immutable default transition, bounded library/custody/transport slices, exact fixed benign composition, and v24 corrected/early-denial guest execution passed. Semantic review marked the socket-creation-only v25 path `NO_GO`; v26 tests local-CID capability without connecting or sending. V26 execution, final typed transport/launcher profile, later denial and hostile corpora, independent product-admission review, installed composition, and final profile reruns remain. |
