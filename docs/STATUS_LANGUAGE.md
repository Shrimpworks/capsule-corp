...
| Archive fixed-store F2 migration, F3 first-segment activation, and F4A read-only lookup | `PASSED` | F2's owner-asserted migration/full verifier, F3's sealed first-segment transaction, and F4A's retained-global lookup/replay/passive-collision/hot-only-recovery routing pass their exact local scopes, including missing-history preservation, publish-before-reference ordering, atomic old-or-new reopen, complete visible tombstones, hot/archive semantic equality, and retained fault/corruption/substitution/concurrency/restoration oracles. F4B mutation/new tombstones, F4C bounded growth, F5+ backup/engine behavior, and production admission remain outside this scope. |
