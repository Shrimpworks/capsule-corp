...
| Fixed denial-test v20 no-launch materialization and runtime stop | Materialization `PASSED`: independent A/B construction reproduced the network-disabled root, guest binary, signed runner, profile, and controller; signatures, digests, profile assertions, and the fixed local-only probe surface passed static validation. Exact runtime attempt `BLOCKED`: the signed runner exited 125 before readiness, with no start authorization or guest launch. | The exact pre-ready check/libkrun stage is unknown because early stderr was not persisted. V23 diagnosis and durable archive publication remain `BLOCKED`; the parent workstream is `IN_PROGRESS — TRENDING_BAD`. |
| Fixed denial-test v21 diagnostic materialization and runtime stop | Materialization `PASSED`: a fixed successor reproducibly added bounded pre-ready stage labels and early-stderr persistence without adding authority. Exact runtime attempt `BLOCKED`: ready EOF arrived before `R`, with no start authorization or guest launch. | The controller selected ready EOF before process wait and failed to persist drained stderr or authoritative waitpid evidence. V23 diagnosis and durable archive publication remain `BLOCKED`; the parent workstream remains `IN_PROGRESS — TRENDING_BAD`. |
| Fixed denial-test v22 convergence materialization and runtime stop | Materialization `PASSED`: a fixed successor reproducibly converged every non-`R` result through bounded exact-child wait, stderr persistence, waitpid recording, and canary verification without adding authority. Exact runtime attempt `BLOCKED`: retained stderr identified `preflight-root-sha256`; no authorization, libkrun configuration, HVF call, or guest launch occurred. | Host-only source-root and fresh-copy hashes match the expected root, so the inherited-FD/hash-state cause remains unresolved. V23 diagnosis and durable archive publication remain `BLOCKED`; the parent workstream remains `IN_PROGRESS — TRENDING_BAD`. |
| Fixed denial-test v23 hash-diagnostic materialization | `PASSED`: a fixed build-only successor reproducibly adds staged-path/open-FD Supervisor hashes and bounded runner hash-stage/mismatch evidence without adding authority. | No runner, libkrun/HVF process, or guest was executed. Dynamic v23 execution is `BLOCKED` on fresh exact one-use authorization naming attempt `capsule-c2b-v23-hostile-preflight-diagnostic-owned-guest-20260806-01` and composed digest `edf89c87...21b9`; durable archive publication also remains `BLOCKED`. |
...
| Governed fork promotions | `PASSED`: Deno r3, `rusty_v8` r5, and libkrun r3 accepted lines are locked defaults with historical refs protected. | Governed runtime engineering is now `IN_PROGRESS — TRENDING_BAD` after the later v20-v22 pre-ready diagnostic stops; release and admission remain `BLOCKED`. |
...
| Fixed denial-test v20-v23 diagnostic sequence | Defensive/local-only research task `019fd84d-396d-7e31-ba8b-3eca9952d554` | V20-v23 no-launch materializations `PASSED`; exact v20-v22 attempts are `BLOCKED`; parent governed runtime/libkrun work is `IN_PROGRESS — TRENDING_BAD`. V22 retained authoritative child/stderr/waitpid/teardown/canary evidence and localized exit 125 to `preflight-root-sha256`, before authorization or libkrun/HVF/guest activity. Host-only hashes match the expected root. V23 reproducibly adds staged-path/open-FD and bounded runner-stage/mismatch hashing without execution or new authority. | [First owned guest execution checkpoint](FIRST_OWNED_GUEST_EXECUTION_CHECKPOINT.md), exact v20-v23 identities and claim boundary; local reports/receipts remain only in the disposable experiment workspace pending archive publication, and v23 execution is `BLOCKED` on fresh exact authorization |
