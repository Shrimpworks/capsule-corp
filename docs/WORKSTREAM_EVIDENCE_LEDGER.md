...
| Fixed denial-test v25 vsock-diagnostic materialization | `PASSED`: a fixed network-disabled build-only successor reproducibly adds distinct socket-open/refusal and device-visibility markers without any connect/send call or new authority. | No runner, libkrun/HVF process, or guest was executed. Dynamic v25 execution is `BLOCKED` on fresh exact one-use authorization naming attempt `capsule-c2b-v25-hostile-vsock-diagnostic-owned-guest-20260806-01` and composed digest `7104377d...ee68`; durable archive publication also remains `BLOCKED`. |
...
| Fixed denial-test v20-v25 diagnostic sequence | Defensive/local-only research task `019fd84d-396d-7e31-ba8b-3eca9952d554` | V20-v25 materializations, v23 root-digest localization, and v24 corrected preflight/known-answer/early denials `PASSED`; complete v24 corpus is `BLOCKED` in the vsock family; parent governed runtime/libkrun work is `IN_PROGRESS — TRENDING_GOOD`. V24 launched one exact owned guest, passed non-root/no-new-privileges/capability/descriptor/root/host-path/mount/root-regain controls, made no connect/send, then stopped before a vsock result marker. V25 reproducibly adds no-network socket/refusal/device markers without execution or new authority. | [First owned guest execution checkpoint](FIRST_OWNED_GUEST_EXECUTION_CHECKPOINT.md), exact v20-v25 identities and claim boundary; local reports/receipts remain only in the disposable experiment workspace pending archive publication, and v25 execution is `BLOCKED` on fresh exact authorization |
