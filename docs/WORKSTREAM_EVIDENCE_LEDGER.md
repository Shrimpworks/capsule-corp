...
| Governed fork promotions | `PASSED`: Deno r3, `rusty_v8` r5, and libkrun r3 accepted lines are locked defaults with historical refs protected. | Governed runtime engineering is now `IN_PROGRESS — TRENDING_BAD` after the later v20/v21 pre-ready diagnostic stops; release and admission remain `BLOCKED`. |
...
| Fixed denial-test v20-v22 diagnostic sequence | Defensive/local-only research task `019fd84d-396d-7e31-ba8b-3eca9952d554` | V20-v22 no-launch materializations `PASSED`; exact v20/v21 attempts are `BLOCKED`; parent governed runtime/libkrun work is `IN_PROGRESS — TRENDING_BAD`. V20 exited 125 pre-ready after artifact preflight. V21 reached ready EOF before `R`; no start authorization, attempt record, guest launch, network authority/traffic, or credential authority occurred, but the controller returned before drained stderr or authoritative waitpid evidence persisted. V22 reproducibly adds bounded child/stderr/waitpid/canary convergence without execution or new authority. | [First owned guest execution checkpoint](FIRST_OWNED_GUEST_EXECUTION_CHECKPOINT.md), exact v20-v22 identities and claim boundary; local reports/receipts remain only in the disposable experiment workspace pending archive publication, and v22 execution is `BLOCKED` on fresh exact authorization |
