...
| Fixed denial-test v24 corrected materialization | `PASSED`: a fixed build-only successor reproducibly replaces the malformed byte array with the exact lowercase root digest plus C17 static length assertion while preserving all authority and teardown boundaries. | No runner, libkrun/HVF process, or guest was executed. Dynamic v24 execution is `BLOCKED` on fresh exact one-use authorization naming attempt `capsule-c2b-v24-hostile-corrected-owned-guest-20260806-01` and composed digest `7849551a...a619`; durable archive publication also remains `BLOCKED`. |
...
| Fixed denial-test v20-v24 diagnostic sequence | Defensive/local-only research task `019fd84d-396d-7e31-ba8b-3eca9952d554` | V20-v24 no-launch materializations and v23 root-digest localization `PASSED`; exact v20-v23 hostile attempts are `BLOCKED`; parent governed runtime/libkrun work is `IN_PROGRESS — TRENDING_GOOD`. V23's exact one-use invocation proved the staged path, unlinked FD, runner-computed digest, and actual root matched; its embedded expected bytes were malformed from byte 18 through 31. No authorization/libkrun/HVF/guest/network/credential activity occurred. V24 reproducibly replaces that array with an exact literal and static length assertion without execution or new authority. | [First owned guest execution checkpoint](FIRST_OWNED_GUEST_EXECUTION_CHECKPOINT.md), exact v20-v24 identities and claim boundary; local reports/receipts remain only in the disposable experiment workspace pending archive publication, and v24 execution is `BLOCKED` on fresh exact authorization |
