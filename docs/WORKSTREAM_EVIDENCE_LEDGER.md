...
| Fixed denial-test v27 passive-network correction | `PASSED`: a fixed network-disabled build-only successor reproducibly accepts only loopback plus optional down/unbacked `dummy0`, rejects virtio backing and non-loopback/default routes, and retains no-connect/no-send behavior. | No runner, libkrun/HVF process, or guest was executed. Dynamic v27 execution is `BLOCKED` on fresh exact one-use authorization naming attempt `capsule-c2b-v27-hostile-network-corrected-owned-guest-20260806-01` and composed digest `52f38c8f...073e`; durable archive publication also remains `BLOCKED`. |
...
| Fixed denial-test v20-v27 diagnostic sequence | Defensive/local-only research task `019fd84d-396d-7e31-ba8b-3eca9952d554` | V20-v27 materializations, v23 root-digest localization, v24 early denials, and v26 consolidated localization `PASSED`; complete corpus remains `BLOCKED`; v25 is `NO_GO`; parent governed runtime/libkrun work is `IN_PROGRESS — TRENDING_GOOD`. V26 passed active local-CID and raw-block denials, then fixed detail exposed expected down `dummy0` as the next probe-policy mismatch without connect/send/traffic. V27 reproducibly accepts only loopback plus down/unbacked `dummy0` and rejects virtio backing and non-loopback/default routes without execution or new authority. | [First owned guest execution checkpoint](FIRST_OWNED_GUEST_EXECUTION_CHECKPOINT.md), exact v20-v27 identities and claim boundary; local reports/receipts remain only in the disposable experiment workspace pending archive publication, and v27 execution is `BLOCKED` on fresh exact authorization |
