...
2. **Source Validator architecture accepted; product blocked:** Accepted ADR-0036 closes R0 with
   ...
   R1 passive v1 contracts/field authority are `PASSED`; product work remains `BLOCKED`. Continue
   only with R2 unsigned construction before separately authorized signing/install, measured
   corpus, or either consumer.
...
7. **Next Source Validator path:** accepted ADR-0034's M1 bytes, Accepted ADR-0035's passive V0
   ...
   claim. R1 passive v1 contracts/field authority are retained; product work remains `BLOCKED` and
   proceeds sequentially: R2 unsigned construction; separately authorized R3 signing/install; R4 confinement/
