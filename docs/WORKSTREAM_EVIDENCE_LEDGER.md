...
| Fixed denial-test v26 consolidated materialization | `PASSED`: a fixed network-disabled build-only successor reproducibly tests `VM_SOCKETS_GET_LOCAL_CID` without connect/send and retains fixed failure detail plus bounded console progress without adding authority. | No runner, libkrun/HVF process, or guest was executed. Dynamic v26 execution is `BLOCKED` on fresh exact one-use authorization naming attempt `capsule-c2b-v26-hostile-consolidated-owned-guest-20260806-01` and composed digest `5f460f89...ff7a`; durable archive publication also remains `BLOCKED`. |
...
| Fixed denial-test v20-v26 diagnostic sequence | Defensive/local-only research task `019fd84d-396d-7e31-ba8b-3eca9952d554` | V20-v26 materializations, v23 root-digest localization, and v24 corrected preflight/known-answer/early denials `PASSED`; complete v24 corpus remains `BLOCKED`; v25 is `NO_GO`; parent governed runtime/libkrun work is `IN_PROGRESS — TRENDING_GOOD`. V24 stopped in the vsock family without connect/send. Semantic review showed v25 tested socket creation rather than usable capability. V26 reproducibly tests the local-CID ioctl and records one fixed failure reason plus bounded console progress without execution or new authority. | [First owned guest execution checkpoint](FIRST_OWNED_GUEST_EXECUTION_CHECKPOINT.md), exact v20-v26 identities and claim boundary; local reports/receipts remain only in the disposable experiment workspace pending archive publication, and v26 execution is `BLOCKED` on fresh exact authorization |
