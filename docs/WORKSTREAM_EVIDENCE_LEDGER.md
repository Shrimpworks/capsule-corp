...
| Fixed denial-test v20 no-launch materialization and runtime stop | Materialization `PASSED`: independent A/B construction reproduced the network-disabled root, guest binary, signed runner, profile, and controller; signatures, digests, profile assertions, and the fixed local-only probe surface passed static validation. Exact runtime attempt `BLOCKED`: the signed runner exited 125 before readiness, with no start authorization or guest launch. | The exact pre-ready check/libkrun stage is unknown because early stderr was not persisted. V21 diagnosis and durable archive publication remain `BLOCKED`; the parent workstream is `IN_PROGRESS — TRENDING_BAD`. |
| Fixed denial-test v21 diagnostic materialization | `PASSED`: a fixed build-only successor reproducibly adds bounded pre-ready stage labels and early-stderr persistence without adding authority. | No runner, libkrun/HVF process, or guest was executed. Dynamic v21 execution is `BLOCKED` on fresh exact one-use authorization naming attempt `capsule-c2b-v21-hostile-preflight-diagnostic-owned-guest-20260806-01` and composed digest `bfe1481b...b5b3`; durable archive publication also remains `BLOCKED`. |
...
| Fixed denial-test v20/v21 diagnostic sequence | Defensive/local-only research task `019fd84d-396d-7e31-ba8b-3eca9952d554` | V20 no-launch materialization `PASSED`, but its exact runtime attempt is `BLOCKED`; parent governed runtime/libkrun work is `IN_PROGRESS — TRENDING_BAD`. All v20 artifact preflights passed before the exact runner exited 125 pre-ready; no start authorization, attempt record, `krun_start_enter`, guest launch, network authority/traffic, or credential authority occurred. Normal reap and unlinked-root teardown passed, but zero console/completion bytes and missing persisted early stderr leave the stage unknown. V21 build-only diagnostics then `PASSED` reproducibility/signature/digest/build/profile checks without execution or new authority. | [First owned guest execution checkpoint](FIRST_OWNED_GUEST_EXECUTION_CHECKPOINT.md), exact v20/v21 identities and claim boundary; local reports/receipts remain only in the disposable experiment workspace pending archive publication, and v21 execution is `BLOCKED` on fresh exact authorization |
