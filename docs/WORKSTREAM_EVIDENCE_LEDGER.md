...
| Fixed denial-test v20 no-launch materialization | `PASSED`: independent A/B construction reproduced the network-disabled root, guest binary, signed runner, profile, and controller; signatures, digests, profile assertions, and the fixed local-only probe surface passed static validation. | No runner, libkrun/HVF process, or guest was launched. Dynamic v20 execution is `BLOCKED` on fresh exact one-use authorization naming attempt `capsule-c2b-v20-hostile-denial-owned-guest-20260806-01` and composed digest `29bfd640...d71a9`; durable archive publication also remains `BLOCKED`. |
...
| Fixed denial-test v20 no-launch materialization | Defensive/local-only research task `019fd84d-396d-7e31-ba8b-3eca9952d554` | Exact no-launch materialization `PASSED`; parent governed runtime/libkrun work remains `IN_PROGRESS — TRENDING_GOOD`; dynamic execution and durable archive publication remain `BLOCKED`. A/B root, guest binary, signed runner, profile, and controller reproduction plus strict signatures, independent digests, fixed profile assertions, and source/control/sink validation bound attempt `capsule-c2b-v20-hostile-denial-owned-guest-20260806-01` to composed digest `29bfd640...d71a9`. No network, credential, external target, runner/libkrun/HVF process, or guest launch occurred. | [First owned guest execution checkpoint](FIRST_OWNED_GUEST_EXECUTION_CHECKPOINT.md), exact v20 hashes and authorization boundary; local report/receipt remain only in the disposable experiment workspace pending archive publication |
