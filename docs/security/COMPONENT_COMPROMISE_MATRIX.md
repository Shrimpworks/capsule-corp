...
| Proposed TypeScript Source Preparer / Node worker | Corrupt original/emitted relationships before registration, lie about preparation, lose or quarantine source sets, cause denial of service | Cannot approve or register a plan, use operational keys, access Broker user-only content, launch a backend, bypass Supervisor hard safety, or substitute bytes without changing registered digests | Disable preparation and attempts needing affected sets, quarantine store/release, repair or forward-update, require new plan/approval for changed bytes |
