...
| Proposed TypeScript Source Preparer native front / Go core / store / Node worker | Before plan construction, choose mutually consistent executable JavaScript, profile/options, records, digests, and prepared-set claims; retain or disclose agent-facing proposal source; exploit an omitted binding; replay within an omitted installation/epoch/intent binding; exhaust bounded availability; corrupt or destroy its own unregistered state. Native front and Go core are one process and therefore one compromise domain. Node/Amaro is also full store/planning TCB until a child profile proves otherwise. | Cannot create Approval or Supervisor-evidence signatures, register or launch a plan, use operational keys, access Broker user-only content, clear quarantine, bypass Supervisor hard safety, or substitute bytes after registration without a digest mismatch. Supervisor/Broker consistency validation does not prove faithful erasure or semantic equivalence. | Stop TypeScript preparation and dependent attempts, preserve/quarantine store and release evidence, revoke the exact component/epoch, repair or forward-update, and require a new plan/approval for changed bytes. If protected-store or worker-confinement probes fail, retain JavaScript-only admission or revise the ADR. |
